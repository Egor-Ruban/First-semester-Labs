		...
		while(a[0]<=size-n&&i>=0){
	    	...
	    	//иначе обрабатываем RE
    			...
    			if(i==n-1){
			    	...
			    	for(int j = RE; j<size;j++){
			    		//if(tarr[0]==F[j]&&tarr[1]==T[j]||tarr[0]==T[j]&&tarr[1]==F[j]){
					if(C1[Q-1]==F[j]&&C2[Q-1]==T[j]||C1[Q-1]==T[j]&&C2[Q-1]==F[j]){
						S = 0;
			    			a[n-1]=j;
			    			for(int k = 0;k<n;k++)
			    				S+=L[a[k]];
				...
				//for(int j = 0; j<n;j++) fprintf(fout,"%d%d, ",F[a[j]],T[a[j]]);
				//printf("S is %d \n",S);
                 //и если сумма получилась меньше найденой ранее - сохраняем
					...
					for(int k = 1;k<n&&check==0;k++){
				            if(F[a[k]]==ta[j-1]&&T[a[k]]!=ta[j-2]){
					   	ta[j]=T[a[k]];
						check++;
					    } else if(T[a[k]]==ta[j-1]&&F[a[k]]!=ta[j-2]){
						ta[j]=F[a[k]];
						check++;
					    }
				        }
